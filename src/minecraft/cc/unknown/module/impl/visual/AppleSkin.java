...
import cc.unknown.module.impl.visual.appleskin.AppleSkinHelper;
import cc.unknown.module.impl.visual.appleskin.FoodValues;
...
public final class AppleSkin extends Module {
	...
	public final Listener<RenderHungerEvent> onHunger = event -> {
		...
		final ItemStack heldItem = mc.player.getHeldItem();
	...
	private void generateHungerBarOffsets(final int right, final int top, final int ticks) {
		...
		final boolean shouldAnimatedFood = saturationLevel <= 0.0F
				&& mc.ingameGUI.updateCounter % (foodLevel * 3 + 1) == 0;
	...
	private void drawSaturationOverlay(final float saturationGained, final float saturationLevel,
			final int hungerRestored, final int foodLevel, final int right, final int top, final float alpha) {
		...
		for (int i = startSaturationBar; i < endSaturationBar; ++i) {
			...
			for (final PotionEffect e : mc.player.getActivePotionEffects()) {
	...
	private void drawHungerOverlay(final int hungerRestored, final int foodLevel, final int right, final int top,
			final float alpha, final boolean useRottenTextures) {
		if (hungerRestored <= 0) {
			return;
		}
