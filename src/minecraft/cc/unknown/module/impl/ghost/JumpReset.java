...
public class JumpReset extends Module {
	...
	private final NumberValue fallDistance = new NumberValue("Fall Distance", this, 3.5, 0, 5, .1);
	...
	public final Listener<MoveInputEvent> onMoveInput = event -> {
		if (!isChance() || shouldPerformAction()) {
			return;
		}
		
		if (getModule(NoClip.class).isEnabled()) return;

		if (mc.player.hurtTime == hurtTime.getValueToInt() && mc.player.fallDistance == fallDistance.getValueToFloat()) {
			event.setJump(true);
		}
	};
