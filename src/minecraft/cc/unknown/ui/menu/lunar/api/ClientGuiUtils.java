package cc.unknown.ui.menu.lunar.api;
    ...
    public static int drawString(String text, int x, int y, int color, boolean shadow) {
        ...
        if (lines.length > 1) {
            ...
            for (int i = 0; i < lines.length; i++)
                j += Minecraft.getInstance().fontRendererObj.draw(lines[i], x, (y + i * (Minecraft.getInstance().fontRendererObj.FONT_HEIGHT + 2)), color, shadow);
            return j;
        }
        return Minecraft.getInstance().fontRendererObj.draw(text, x, y, color, shadow);
    }
