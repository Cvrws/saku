...
public class SimulatedPlayer implements Accessor {
	...
	public SimulatedPlayer(EntityPlayerSP player, AxisAlignedBB box, MovementInput movementInput, int jumpTicks,
			...
			boolean isCollidedVertically, WorldBorder worldBorder, IChunkProvider chunkProvider,
			boolean isOutsideBorder, Entity riddenByEntity, BaseAttributeMap attributeMap, boolean isSpectator,
			float fallDistance, float stepHeight, boolean isCollided, int fire, float distanceWalkedModified,
			float distanceWalkedOnStepModified, int nextStepDistance, float height, float width, int fireResistance,
			boolean isInWeb, boolean noClip, boolean isSprinting, FoodStats foodStats) {
		this.player = player;
		this.box = box;
	...
	public static SimulatedPlayer fromClientPlayer(MovementInput input) {
		...
		return new SimulatedPlayer(player, player.getEntityBoundingBox(), movementInput, player.jumpTicks,
				...
				player.worldObj.getWorldBorder(), player.worldObj.getChunkProvider(), player.isOutsideBorder(),
				player.riddenByEntity, player.getAttributeMap(), player.isSpectator(), player.fallDistance,
				player.stepHeight, player.isCollided, player.fire, player.distanceWalkedModified,
				player.distanceWalkedOnStepModified, player.nextStepDistance, player.height, player.width,
				player.fireResistance, player.isInWeb, player.noClip, player.isSprinting(), foodStats);
	}
	...
	private void clientPlayerLivingUpdate() {
		...
		if (onGround && movementInput.moveForward >= f && !isSprinting() && flag3 && !player.isUsingItem()
				&& !isPotionActive(Potion.blindness) && shouldSprint) {
			setSprinting(true);
		}

		if (!isSprinting() && movementInput.moveForward >= f && flag3 && !player.isUsingItem()
				&& !isPotionActive(Potion.blindness) && shouldSprint) {
			setSprinting(true);
		}
		...
		if (isServerWorld()) {
			...
			if (!isInWater() || this.capabilities.isFlying) {
				if (isInLava() && !this.capabilities.isFlying) {
					...
					if (worldObj.isRemote && (!worldObj.isBlockLoaded(new BlockPos(posX, 0, posZ))
							|| !worldObj.getChunkFromBlockCoords(new BlockPos(posX, 0, posZ)).isLoaded())) {
	...
	public void doBlockCollisions() {
		BlockPos blockpos = new BlockPos(getEntityBoundingBox().minX + 0.001, getEntityBoundingBox().minY + 0.001,
				getEntityBoundingBox().minZ + 0.001);
		BlockPos blockpos1 = new BlockPos(getEntityBoundingBox().maxX - 0.001, getEntityBoundingBox().maxY - 0.001,
				getEntityBoundingBox().maxZ - 0.001);
		if (isAreaLoaded(blockpos.getX(), blockpos.getY(), blockpos.getZ(), blockpos1.getX(), blockpos1.getY(),
				blockpos1.getZ(), true)) {
	...
	public boolean handleWaterMovement() {
		if (handleMaterialAcceleration(
				getEntityBoundingBox().expand(0.0, -0.4000000059604645, 0.0).contract(0.001, 0.001, 0.001),
				Material.water)) {
			fallDistance = 0.0f;
			inWater = true;
			fire = 0;
	...
	public boolean isInLava() {
		return worldObj.isMaterialInBB(
				getEntityBoundingBox().expand(-0.10000000149011612, -0.4000000059604645, -0.10000000149011612),
				Material.lava);
	}
	...
	private boolean isRainingAt(BlockPos pos) {
		if (worldObj.getRainStrength(1.0F) <= 0.2) {
			...
			if (base.enableSnow)
				return false;
			else if (worldObj.canSnowAt(pos, false))
				return false;
			else
				return base.canRain();
		}
	...
	public Pair<Double, Double> checkForCollision(SimulatedPlayer simPlayer, double velocityX, double velocityZ) {
		...
		for (d6 = 0.05; velocityX != 0 && worldObj
				.getCollidingBoundingBoxes(player, simPlayer.box.offset(velocityX, -1, 0)).isEmpty(); d3 = velocityX) {
		...
		for (; velocityZ != 0 && worldObj.getCollidingBoundingBoxes(player, simPlayer.box.offset(0, -1, velocityZ))
				.isEmpty(); d5 = velocityZ) {
		...
		for (; velocityX != 0 && velocityZ != 0
				&& worldObj.getCollidingBoundingBoxes(player, simPlayer.box.offset(velocityX, -1, velocityZ))
						.isEmpty(); d5 = velocityZ) {
			if (velocityX < d6 && velocityX >= -d6) {
				velocityX = 0;
			} else if (velocityX > 0) {
